...
class HashCheckingVisitor implements FileVisitor<Path> {
	...
	HashCheckingVisitor(FileHashStoreFactory factory, FileHashGenerator hashGenerator, String[] excludedExtensions) {
	...
	public FileVisitResult preVisitDirectory(Path path, BasicFileAttributes attrs) throws IOException {
		try {
			...
			actions.add(
					new WarningAction(
							String.format("Error caught visiting directory %s this directory will be ignored. %s", path,
									e)));
		}
	...
	public FileVisitResult postVisitDirectory(Path path, IOException e) throws IOException {
		if (e != null) {
			actions.add(
					new WarningAction(
							String.format(
									"Directory %s not fully scanned, some files may not have hashes and some subdirectories may not be scanned. %s",
									path, e)));
		}
	...
	public FileVisitResult visitFile(Path filePath, BasicFileAttributes attrs) throws IOException {
		if (FileHelper.fileEndsWith(filePath, excludedExtensions)) {
			return FileVisitResult.CONTINUE;
		}
	...
	private void calculateAndStoreHash(Path path, FileHashStore store) throws IOException {
		...
		if (!store.hashExists(fileName)) {
			...
			actions.add(
					new WarningAction(
							String.format("Calculated hash (%s) for file %s does not match stored value (%s)",
									calculatedHash, path, store.getHash(fileName))));
		}
	}
